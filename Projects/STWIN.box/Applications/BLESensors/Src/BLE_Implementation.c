...
void BluetoothInit(void)
{
  ...
  /* Disabling the Secure Connection */
  BLE_StackValue.EnableSecureConnection=0;
  /* Force BLE Rescan at Connection */
...
static void ReadRequestEnvFunction(int32_t *Press,uint16_t *Hum,int16_t *Temp1,int16_t *Temp2)
{
  ...
  BSP_ENV_SENSOR_GetValue(STTS22H_0, ENV_TEMPERATURE, &Temperature);
  BSP_ENV_SENSOR_GetValue(ILPS22QS_0, ENV_PRESSURE, &Pressure);

  *Press = (int32_t)(Pressure *100);
  *Temp1 = (int16_t)(Temperature * 10);
}
/**
